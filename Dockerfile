...
# renovate: datasource=deb depName=samba-ad-dc extractVersion=^\d:(?<version>[\d\.]+)(\+dfsg-(?<build>\d+))?(ubuntu(?<revision>\d*))?$ registryUrl=http://ports.ubuntu.com/ubuntu-ports?release=noble&components=universe&binaryArch=arm64
